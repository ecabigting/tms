...
export default function TaskForm({ task, userList }: any) {
	const { toast } = useToast();
	const [error, setError] = useState<string | undefined>("");
	const [success, setSuccess] = useState<string | undefined>("");
	const [isPending, startTransition] = useTransition();
	// const [dueDate, setDueDate] = useState<Date>(new Date(task.dueDate));
	const [assignToName, setAssignToName] = useState(task.assignedToName);
	const [assignToId, setAssignToId] = useState(task.assignedToId);
	...
	const form = useForm<z.infer<typeof TaskSchema>>({
		...
		defaultValues: { ...task, dueDate: new Date(task.dueDate || new Date()) },
	});
